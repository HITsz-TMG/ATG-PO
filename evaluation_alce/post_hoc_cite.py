...
def main():
    ...
    if "gtr" in args.retriever:
        ...
        gtr_model = SentenceTransformer(args.retriever, device=args.retriever_device)
    ...
    for idx, item in enumerate(tqdm(data['data'])):
        ...
        for sent in sents:
            ...
            if len(original_ref) == 0 or args.overwrite:
                ...
                if best_doc_id not in mapping.keys():
                    mapping[best_doc_id] = doc_start_idx
                doc_start_idx = doc_start_idx + 1
                sent = f"[{mapping[best_doc_id]+1}] " + sent
        ...
        item['docs'] = [doc_list[i] for i in mapping.values()]
        new_data.append(item)

    data['data'] = new_data 
    tag = "gtr-t5-large-external"
    if args.overwrite:
        tag += "-overwrite"
    if args.external_docs is not None:
        tag += "-external"
