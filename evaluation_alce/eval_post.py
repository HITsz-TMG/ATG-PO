...
def compute_autoais(data,
    ...
    for item in tqdm(data):
        ...
        if qampari:
            ...
            if "<|endofstatement|>" in item["output"] or "endofselection" in item["output"]:
                sents = item["output"].split("<|endofstatement|>")
            elif mode == 'nltk':
                sents = sent_tokenize(item['output'])
            elif mode == 'stanza':
                doc = nlp(item['output'])
        ...
        sents  = [sent.replace("<|beginofselection|>", "") for sent in sents]
        sents = [sent.replace("<|endofselection|>", "") for sent in sents]
...
def main():
    ...
    parser.add_argument("--mode", type=str, default='nltk', help="nltk or stanza")
