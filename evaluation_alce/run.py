...
class LLM:
    ...
    def __init__(self, args):
        ...
        if args.openai_api:
            ...
            self.model, self.tokenizer = load_model(args.model, args=self.args)
    ...
    def generate(self, prompt, max_tokens, stop=None):
        ...
        if args.openai_api:
            ...
            generation = self.tokenizer.decode(outputs[0][inputs['input_ids'].size(1):], skip_special_tokens=False)
...
def main():
    ...
    json.dump(eval_data, open(args.save_folder + '/' + name + ".json", "w"), indent=4)
